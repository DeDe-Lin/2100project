...
function Get-FreeSummaryHtml {
    ...
    try {
        ...
        $driveParts = $disks | ForEach-Object {
            ...
            $size = $_.SizeGB
            $pct = $_.UsedPct
            if ([double]$pct -ge $threshold) {
                # 用紅色加粗標註百分比
                "$letter：$used/$size/<span style='color:red;font-weight:bold;'>$pct`%</span>"
            } else {
                "$letter：$used/$size/$pct`%"
            }
        }

        $summary = "$Alias($Computer)-本次剩餘空間 " + ($driveParts -join ', ')
        # 放在表格一列
...
$bodyBuilder += "<tr><th>主機</th><th>磁碟狀況</th></tr>"
